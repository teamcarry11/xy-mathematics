...
const SecurityManager = grain_core.security_manager.SecurityManager;
const PermissionType = grain_core.security_manager.PermissionType;
const UserRole = grain_core.security_manager.UserRole;
...
test "security manager constants" {
    std.debug.assert(grain_core.security_manager.MAX_PERMISSIONS == 64);
    std.debug.assert(grain_core.security_manager.MAX_PERMISSION_NAME_LEN == 64);
    std.debug.assert(grain_core.security_manager.MAX_USERS == 32);
    std.debug.assert(grain_core.security_manager.MAX_USER_NAME_LEN == 64);
}

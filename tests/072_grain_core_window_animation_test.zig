...
const AnimationManager = grain_core.window_animation.AnimationManager;
const AnimationType = grain_core.window_animation.AnimationType;
...
test "animation constants" {
    std.debug.assert(grain_core.window_animation.MAX_ANIMATIONS == 64);
    std.debug.assert(grain_core.window_animation.ANIMATION_DURATION_MS == 200);
}

...
const ConfigParser = grain_core.runtime_config.ConfigParser;
const ConfigCommand = grain_core.runtime_config.ConfigCommand;
const RuntimeConfig = grain_core.runtime_config.RuntimeConfig;
...
test "config parser parse layout type" {
    ...
    std.debug.assert(layout1.? == grain_core.layout_generator.LayoutType.tall);

    const layout2 = ConfigParser.parse_layout_type("wide");
    std.debug.assert(layout2 != null);
    std.debug.assert(layout2.? == grain_core.layout_generator.LayoutType.wide);

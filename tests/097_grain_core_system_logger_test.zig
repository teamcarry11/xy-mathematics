...
const SystemLogger = grain_core.system_logger.SystemLogger;
const LogLevel = grain_core.system_logger.LogLevel;
...
test "system logger constants" {
    std.debug.assert(grain_core.system_logger.MAX_LOG_ENTRIES == 1024);
    std.debug.assert(grain_core.system_logger.MAX_LOG_MESSAGE_LEN == 512);
    std.debug.assert(grain_core.system_logger.MAX_LOG_SOURCE_LEN == 64);
}

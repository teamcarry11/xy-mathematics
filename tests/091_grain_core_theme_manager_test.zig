...
const ThemeManager = grain_core.theme_manager.ThemeManager;
...
test "theme constants" {
    std.debug.assert(grain_core.theme_manager.MAX_THEMES == 16);
    std.debug.assert(grain_core.theme_manager.MAX_THEME_NAME_LEN == 64);
    std.debug.assert(grain_core.theme_manager.MAX_COLOR_LEN == 8);
}

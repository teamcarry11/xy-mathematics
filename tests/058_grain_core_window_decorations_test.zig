...
test "title bar hit testing" {
    ...
    if (compositor.get_window(window_id)) |win| {
        ...
        const title_x = win_x + grain_core.compositor.BORDER_WIDTH + 10;
        const title_y = win_y + grain_core.compositor.BORDER_WIDTH + 5;
...
test "close button hit testing" {
    ...
    if (compositor.get_window(window_id)) |win| {
        const win_x = @as(u32, @intCast(win.x));
        const win_y = @as(u32, @intCast(win.y));
        const button_x = win_x + win.width - grain_core.compositor.BORDER_WIDTH - 10;
        const button_y = win_y + grain_core.compositor.BORDER_WIDTH + 5;

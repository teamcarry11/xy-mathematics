...
pub fn mobile_auth_middleware(
    ...
    var jwt_token: [grain_core_auth.MAX_JWT_LEN]u8 = undefined;
    const jwt_token_len = auth_integration.extract_jwt_token_from_request(
        request,
        &jwt_token,
    );
    if (jwt_token_len == 0) {
        response.status = 401;
        _ = integration.add_response_header(response, "Content-Type", "application/json");
        const error_json = "{\"error\":\"unauthorized\",\"message\":\"Invalid token\"}";
        _ = integration.set_response_body_json(response, error_json);
        return false;
    }
    var claims: grain_core_auth.JwtClaims = undefined;
    const current_time = std.time.timestamp();
    if (!auth_integration.validate_jwt_token(
        jwt_token[0..jwt_token_len],
        @intCast(current_time),
        &claims,
    )) {
        response.status = 401;
        _ = integration.add_response_header(response, "Content-Type", "application/json");
        const error_json = "{\"error\":\"unauthorized\",\"message\":\"Invalid or expired token\"}";
        _ = integration.set_response_body_json(response, error_json);
        return false;
    }
    std.debug.assert(jwt_token_len > 0);
    std.debug.assert(jwt_token_len <= grain_core_auth.MAX_JWT_LEN);
    return true;
}

...
fn parse_args(
    ...
    while (args.next()) |arg| {
            ...
            std.mem.eql(u8, arg, "-h"))
        {
            ...
            if (args.next()) |val| {
                opts.year = try std.fmt.parseInt(u16, val, 10);
            }
        } else if (std.mem.eql(u8, arg, "-m") or std.mem.eql(u8, arg, "--month")) {
            if (args.next()) |val| {
                opts.month = try std.fmt.parseInt(u8, val, 10);
            }
        } else if (std.mem.eql(u8, arg, "-d") or std.mem.eql(u8, arg, "--day")) {
            if (args.next()) |val| {
                opts.day = try std.fmt.parseInt(u8, val, 10);
            }
        } else if (std.mem.eql(u8, arg, "--hour")) {
            if (args.next()) |val| {
                opts.hour = try std.fmt.parseInt(u8, val, 10);
            }
        } else if (std.mem.eql(u8, arg, "--minute")) {
            if (args.next()) |val| {
                opts.minute = try std.fmt.parseInt(u8, val, 10);
            }
        } else if (std.mem.eql(u8, arg, "-t") or std.mem.eql(u8, arg, "--timezone")) {
            opts.timezone = args.next();
        } else if (std.mem.eql(u8, arg, "-n") or std.mem.eql(u8, arg, "--nakshatra")) {
            opts.moon_nakshatra = args.next();
        } else if (std.mem.eql(u8, arg, "-s") or std.mem.eql(u8, arg, "--sign")) {
            opts.asc_sign = args.next();
        } else if (std.mem.eql(u8, arg, "--degrees")) {
            if (args.next()) |val| {
                opts.asc_degrees = try std.fmt.parseInt(u8, val, 10);
            }
        } else if (std.mem.eql(u8, arg, "--house")) {
            if (args.next()) |val| {
                opts.sun_house = try std.fmt.parseInt(u8, val, 10);
            }
        }
    }
...
fn run_interactive(
    ...
    // Interactive mode disabled for now due to std.io issues
    std.debug.print("Interactive mode not supported in this environment.\n", .{});

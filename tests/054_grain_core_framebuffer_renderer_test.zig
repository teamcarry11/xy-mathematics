...
const FramebufferRenderer = grain_core.framebuffer_renderer.FramebufferRenderer;
...
test "framebuffer renderer clear" {
    var renderer = FramebufferRenderer.init();
    renderer.set_syscall_fn(mock_syscall);
    renderer.clear(grain_core.framebuffer_renderer.COLOR_DARK_BG);
    std.debug.assert(renderer.syscall_fn != null);
}
...
test "framebuffer renderer draw pixel" {
    var renderer = FramebufferRenderer.init();
    renderer.set_syscall_fn(mock_syscall);
    renderer.draw_pixel(100, 200, grain_core.framebuffer_renderer.COLOR_WHITE);
    std.debug.assert(renderer.syscall_fn != null);
}
...
test "framebuffer renderer draw rect" {
    var renderer = FramebufferRenderer.init();
    renderer.set_syscall_fn(mock_syscall);
    renderer.draw_rect(10, 20, 100, 50, grain_core.framebuffer_renderer.COLOR_BLUE);
    std.debug.assert(renderer.syscall_fn != null);
}
...
test "framebuffer renderer draw rect bounds" {
    ...
    renderer.draw_rect(1000, 700, 100, 100, grain_core.framebuffer_renderer.COLOR_RED);
    std.debug.assert(renderer.syscall_fn != null);
}
...
test "framebuffer renderer draw rect negative coords" {
    ...
    renderer.draw_rect(-10, -20, 100, 50, grain_core.framebuffer_renderer.COLOR_GREEN);
    std.debug.assert(renderer.syscall_fn != null);
}

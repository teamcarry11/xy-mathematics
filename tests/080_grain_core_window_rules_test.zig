...
const WindowRuleManager = grain_core.window_rules.WindowRuleManager;
const MatchType = grain_core.window_rules.MatchType;
const ActionType = grain_core.window_rules.ActionType;
...
test "window rules constants" {
    std.debug.assert(grain_core.window_rules.MAX_RULES == 64);
    std.debug.assert(grain_core.window_rules.MAX_PATTERN_LEN == 256);
}

...
const HealthMonitor = grain_core.health_monitor.HealthMonitor;
const HealthStatus = grain_core.health_monitor.HealthStatus;
...
test "health monitor constants" {
    std.debug.assert(grain_core.health_monitor.MAX_HEALTH_CHECKS == 32);
    std.debug.assert(grain_core.health_monitor.MAX_HEALTH_CHECK_NAME_LEN == 64);
    std.debug.assert(grain_core.health_monitor.MAX_HEALTH_CHECK_MSG_LEN == 256);
}

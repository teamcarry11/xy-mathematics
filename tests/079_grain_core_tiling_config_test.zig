...
const TilingConfig = grain_core.tiling_config.TilingConfig;
const LayoutType = grain_core.layout_generator.LayoutType;

test "tiling config initialization" {
    const config = TilingConfig.init();
    std.debug.assert(config.window_gap == grain_core.tiling_config.DEFAULT_WINDOW_GAP);
    std.debug.assert(config.split_ratio == grain_core.tiling_config.DEFAULT_SPLIT_RATIO);
...
test "clamp split ratio" {
    const ratio_low = grain_core.tiling_config.clamp_split_ratio(0.05);
    std.debug.assert(ratio_low == grain_core.tiling_config.MIN_SPLIT_RATIO);
    const ratio_high = grain_core.tiling_config.clamp_split_ratio(0.95);
    std.debug.assert(ratio_high == grain_core.tiling_config.MAX_SPLIT_RATIO);
    const ratio_valid = grain_core.tiling_config.clamp_split_ratio(0.5);
...
test "tiling config constants" {
    std.debug.assert(grain_core.tiling_config.DEFAULT_WINDOW_GAP == 4);
    std.debug.assert(grain_core.tiling_config.DEFAULT_SPLIT_RATIO == 0.5);
    std.debug.assert(grain_core.tiling_config.MIN_SPLIT_RATIO == 0.1);
    std.debug.assert(grain_core.tiling_config.MAX_SPLIT_RATIO == 0.9);
}

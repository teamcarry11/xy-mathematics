...
const Timer = @import("../src/kernel/timer.zig").Timer;
const BasinKernel = @import("../src/kernel/basin_kernel.zig").BasinKernel;
const ClockId = @import("../src/kernel/basin_kernel.zig").ClockId;
...
test "timer init" {
    ...
    try std.testing.expect(timer.last_timer_ns == 0);
}
...
test "timer uptime increases" {
    ...
    const monotonic = timer.get_monotonic_ns();
...
test "clock_gettime syscall" {
    ...
    const seconds: u64 = monotonic_ns / 1000000000;
...
test "sleep_until syscall validation" {
    ...
    const past_timestamp: u64 = if (current_time > 1000000000)
...
test "timer set_timer values" {
    ...
    timer.set_timer(60000000000);
    try std.testing.expect(timer.last_timer_ns == 60000000000);
    
    // Test: Set timer to 1 hour.
    timer.set_timer(3600000000000);
    try std.testing.expect(timer.last_timer_ns == 3600000000000);
}
...
test "timer monotonic consistency" {
    ...
    const t1 = timer.get_monotonic_ns();
    const t2 = timer.get_monotonic_ns();
    const t3 = timer.get_monotonic_ns();

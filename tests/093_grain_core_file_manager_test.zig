...
const FileManager = grain_core.file_manager.FileManager;
const FileType = grain_core.file_manager.FileType;
...
test "file manager constants" {
    std.debug.assert(grain_core.file_manager.MAX_FILES == 512);
    std.debug.assert(grain_core.file_manager.MAX_PATH_LEN == 512);
    std.debug.assert(grain_core.file_manager.MAX_NAME_LEN == 256);
}

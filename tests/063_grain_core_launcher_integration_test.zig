...
const DesktopShell = grain_core.desktop_shell.DesktopShell;
...
test "desktop shell get launcher item at" {
    ...
    const launcher_x = (compositor.output.width - grain_core.desktop_shell.LAUNCHER_WIDTH) / 2;
    const launcher_y = (compositor.output.height - grain_core.desktop_shell.LAUNCHER_HEIGHT) / 2;
...
test "compositor launcher item click" {
    ...
    var input_handler = grain_core.input_handler.InputHandler.init();

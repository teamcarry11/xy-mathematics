...
const SystemDiagnostics = grain_os.system_diagnostics.SystemDiagnostics;
const DiagnosticSeverity = grain_os.system_diagnostics.DiagnosticSeverity;
...
test "system diagnostics constants" {
    std.debug.assert(grain_os.system_diagnostics.MAX_DIAGNOSTIC_CHECKS == 32);
    std.debug.assert(grain_os.system_diagnostics.MAX_CHECK_NAME_LEN == 64);
    std.debug.assert(grain_os.system_diagnostics.MAX_DIAGNOSTIC_MSG_LEN == 256);
}

...
const ServiceManager = grain_core.service_manager.ServiceManager;
const ServiceState = grain_core.service_manager.ServiceState;
const ServiceType = grain_core.service_manager.ServiceType;
...
test "service manager constants" {
    std.debug.assert(grain_core.service_manager.MAX_SERVICES == 64);
    std.debug.assert(grain_core.service_manager.MAX_SERVICE_NAME_LEN == 128);
    std.debug.assert(grain_core.service_manager.MAX_SERVICE_DESC_LEN == 256);
    std.debug.assert(grain_core.service_manager.MAX_DEPENDENCIES == 16);
}

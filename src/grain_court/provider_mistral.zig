...
// Bounded: Max JSON body size for Mistral requests.
const MAX_JSON_BODY_SIZE: u32 = 65536;

// Build Mistral JSON request body.
fn build_mistral_json_body(
...
pub const MistralProvider = struct {
    ...
    pub fn init(
        ...
        provider.trait.provider_type = .mistral;
    ...
    fn send_request_impl(
        ...
        const body_len = try build_mistral_json_body(request, &json_body);
        var i: u32 = 0;
        ...
        if (http_req.response) |http_resp| {
            ...
            try parse_mistral_response(resp_body, &response);
        }

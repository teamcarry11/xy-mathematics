...
const MetricsAggregator = grain_os.system_metrics.MetricsAggregator;
const SystemHealth = grain_os.system_metrics.SystemHealth;
...
test "compositor get overall system health" {
    ...
    if (comp.add_health_check("test")) |check_id| {
        _ = comp.update_health_check(check_id, grain_os.health_monitor.HealthStatus.healthy, "OK", 1000);
    }
    comp.update_system_metrics(1000);

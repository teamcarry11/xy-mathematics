...
const TimeManager = grain_core.time_manager.TimeManager;
const TimeFormat = grain_core.time_manager.TimeFormat;
const DateFormat = grain_core.time_manager.DateFormat;
...
test "time manager constants" {
    std.debug.assert(grain_core.time_manager.MAX_TIMEZONE_NAME_LEN == 64);
    std.debug.assert(grain_core.time_manager.MAX_TIMEZONE_ABBR_LEN == 8);
}

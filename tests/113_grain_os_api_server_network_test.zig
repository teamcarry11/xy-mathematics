...
test "network server init" {
    const network = root.grain_os.api_server_network;
...
test "process http request" {
    const network = root.grain_os.api_server_network;
    const api_server_mod = root.grain_os.api_server;
    const conn_mgr = root.grain_os.connection_manager;
...
fn test_handler(_req: *root.grain_os.api_server.HttpRequest, res: *root.grain_os.api_server.HttpResponse) void {
    _ = _req;
    res.status = root.grain_os.api_server.HttpStatus.ok;

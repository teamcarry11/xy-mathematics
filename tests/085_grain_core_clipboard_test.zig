...
const ClipboardManager = grain_core.clipboard.ClipboardManager;
const ClipboardFormat = grain_core.clipboard.ClipboardFormat;
...
test "clipboard constants" {
    std.debug.assert(grain_core.clipboard.MAX_CLIPBOARD_SIZE == 4096);
    std.debug.assert(grain_core.clipboard.MAX_HISTORY_ENTRIES == 16);
    std.debug.assert(grain_core.clipboard.MAX_FORMAT_NAME_LEN == 32);
}

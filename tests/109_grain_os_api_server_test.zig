...
const api_server = @import("grain_os").api_server;
...
test "register server process" {
    ...
    _ = server.start();
    const process_manager = @import("root").grain_os.process_manager.ProcessManager.init();
...
test "update server process state" {
    ...
    _ = server.register_route(api_server.HttpMethod.get, "/api/v1/test", handler);
    _ = server.start();
    var process_manager = @import("root").grain_os.process_manager.ProcessManager.init();
    if (server.register_server_process(&process_manager, 0, "api_server", "api_server --port 8080", 1000)) |_pid| {
        const updated = server.update_server_process_state(
            &process_manager,
            @import("root").grain_os.process_manager.ProcessState.sleeping,
        );

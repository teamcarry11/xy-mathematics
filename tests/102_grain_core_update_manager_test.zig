...
const UpdateManager = grain_core.update_manager.UpdateManager;
const UpdateType = grain_core.update_manager.UpdateType;
const UpdateState = grain_core.update_manager.UpdateState;
...
test "update manager constants" {
    std.debug.assert(grain_core.update_manager.MAX_UPDATES == 16);
    std.debug.assert(grain_core.update_manager.MAX_VERSION_LEN == 32);
    std.debug.assert(grain_core.update_manager.MAX_UPDATE_DESC_LEN == 256);
    std.debug.assert(grain_core.update_manager.MAX_UPDATE_URL_LEN == 512);
}

...
const LockScreenManager = grain_core.lock_screen.LockScreenManager;
const IdentityManager = grain_core.lock_screen.IdentityManager;
...
test "lock screen constants" {
    std.debug.assert(grain_core.lock_screen.MAX_IDENTITIES == 16);
    std.debug.assert(grain_core.lock_screen.MAX_IDENTITY_NAME_LEN == 64);
    std.debug.assert(grain_core.lock_screen.MAX_PASSWORD_LEN == 256);
}

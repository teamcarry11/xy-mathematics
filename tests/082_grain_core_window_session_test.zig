...
const SessionManager = grain_core.window_session.SessionManager;
...
test "window session constants" {
    std.debug.assert(grain_core.window_session.MAX_SESSIONS == 32);
    std.debug.assert(grain_core.window_session.MAX_SESSION_NAME_LEN == 64);
}

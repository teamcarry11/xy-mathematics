...
const EventManager = grain_core.window_events.EventManager;
const EventType = grain_core.window_events.EventType;

var test_event_received: bool = false;
var test_event_window_id: u32 = 0;

fn test_listener(event: *const grain_core.window_events.WindowEvent, _user_data: ?*anyopaque) void {
    _ = _user_data;
...
test "window events constants" {
    std.debug.assert(grain_core.window_events.MAX_EVENTS == 128);
    std.debug.assert(grain_core.window_events.MAX_LISTENERS == 16);
}

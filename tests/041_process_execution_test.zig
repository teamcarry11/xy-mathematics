...
const process_execution = @import("basin_kernel").process_execution;
...
test "execute process runs VM until halted or max steps" {
    ...
    const should_continue = process_execution.execute_process(&vm, &context, 100);
...
test "multiple context switches work correctly" {
    ...
    // Switch to context1.
    process_execution.switch_to_process_context(&vm, &context1);
    try testing.expect(vm.regs.pc == 0x10000);
    try testing.expect(vm.regs.get(2) == 0x400000);
    ...
    // Save context1.
    process_execution.save_process_context(&vm, &context1);
    
    // Switch to context2.
    process_execution.switch_to_process_context(&vm, &context2);
    ...
    // Save context2.
    process_execution.save_process_context(&vm, &context2);

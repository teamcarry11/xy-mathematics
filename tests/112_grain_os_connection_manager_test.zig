...
test "connection manager init" {
    const conn_mgr = root.grain_os.connection_manager.ConnectionManager.init();
    std.debug.assert(conn_mgr.connections_len == 0);
    std.debug.assert(conn_mgr.next_connection_id == 1);
}

test "add connection" {
    var conn_mgr = root.grain_os.connection_manager.ConnectionManager.init();
    if (conn_mgr.add_connection()) |idx| {
        ...
        if (conn_mgr.get_connection(idx)) |conn| {
            ...
            std.debug.assert(conn.state == root.grain_os.connection_manager.ConnectionState.idle);
        } else {
...
test "connection keep alive" {
    ...
    if (conn_mgr.add_connection()) |idx| {
        if (conn_mgr.get_connection(idx)) |conn| {
            ...
            std.debug.assert(conn.state == root.grain_os.connection_manager.ConnectionState.keep_alive);
        } else {
            std.debug.assert(false);
        }
    } else {
...
test "connection timeout" {
    ...
    if (conn_mgr.add_connection()) |idx| {
        if (conn_mgr.get_connection(idx)) |conn| {
            ...
            conn_mgr.update_time(1000 + root.grain_os.connection_manager.DEFAULT_REQUEST_TIMEOUT + 1);
...
test "max connections" {
    var conn_mgr = root.grain_os.connection_manager.ConnectionManager.init();
    var i: u32 = 0;
    while (i < root.grain_os.connection_manager.MAX_CONNECTIONS) : (i += 1) {
    ...
    std.debug.assert(conn_mgr.connections_len == root.grain_os.connection_manager.MAX_CONNECTIONS);
    if (conn_mgr.add_connection()) |_idx| {
        std.debug.assert(false);
    }
}

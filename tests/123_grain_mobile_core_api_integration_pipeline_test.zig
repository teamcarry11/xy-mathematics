...
test "register mobile endpoints and test register endpoint" {
    var compositor = grain_core.Compositor.init();
    ...
    std.debug.assert(response.status == grain_core.api_server.HttpStatus.ok or response.status == grain_core.api_server.HttpStatus.bad_request);
}
...
test "test 404 for unregistered route" {
    ...
    std.debug.assert(response_opt != null);
    const response = response_opt.?;
    std.debug.assert(response.status == grain_core.api_server.HttpStatus.not_found);
}
...
test "test middleware execution with CORS" {
    ...
    _ = compositor.api_server.add_middleware_to_route(
        ...
        grain_core.middleware.cors_middleware,
    );
...
test "test request parsing with query parameters" {
    ...
    std.debug.assert(request.method == grain_core.api_server.HttpMethod.get);

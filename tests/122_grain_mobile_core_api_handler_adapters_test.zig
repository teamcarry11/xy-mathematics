...
const grain_mobile = @import("grain_mobile_core");
...
test "handler adapter register endpoint" {
    ...
    var api_server = grain_os.api_server.ApiServer.init(8080);
    ...
    try testing.expect(response.status == grain_os.api_server.HttpStatus.ok);
...
test "handler adapter users profile endpoint" {
    ...
    handler_adapters.set_api_server(&api_server);
    _ = api_server.register_route(
        grain_os.api_server.HttpMethod.get,
...
test "handler adapter bad request handling" {
    ...
    try testing.expect(response_opt != null);
    const response = response_opt.?;
    try testing.expect(response.status == grain_os.api_server.HttpStatus.bad_request);
}

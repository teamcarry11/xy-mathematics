...
const DropZoneManager = grain_core.window_drag_drop.DropZoneManager;
const DropZoneType = grain_core.window_drag_drop.DropZoneType;
...
test "can drag window" {
    std.debug.assert(grain_core.window_drag_drop.can_drag_window(1, false) == true);
    std.debug.assert(grain_core.window_drag_drop.can_drag_window(1, true) == false);
}
...
test "can drop window" {
    ...
    if (manager.find_drop_zone_at(250, 400)) |zone| {
        const result = grain_core.window_drag_drop.can_drop_window(1, zone);
        std.debug.assert(result);
    }
}
...
test "drop zone constants" {
    std.debug.assert(grain_core.window_drag_drop.MAX_DROP_ZONES == 16);
}

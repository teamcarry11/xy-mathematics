...
const window_snapping = grain_core.window_snapping;
...
test "calculate snap position left" {
    ...
    std.debug.assert(snapped.x == @as(i32, @intCast(grain_core.compositor.BORDER_WIDTH)));
...
test "apply snap left" {
    ...
    std.debug.assert(win_x == @as(i32, @intCast(grain_core.compositor.BORDER_WIDTH)));

...
const window_decorations = grain_core.window_decorations;
...
test "get close button bounds" {
    ...
    std.debug.assert(bounds.height == grain_core.compositor.TITLE_BAR_HEIGHT);
}
...
test "compositor is in title bar" {
    ...
    if (comp.get_window(window_id)) |win| {
        const title_bar_x = @as(u32, @intCast(win.x + @as(i32, @intCast(grain_core.compositor.BORDER_WIDTH))));
        const title_bar_y = @as(u32, @intCast(win.y + @as(i32, @intCast(grain_core.compositor.BORDER_WIDTH))));

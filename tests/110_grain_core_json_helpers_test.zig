...
test "find json key" {
    ...
    const json_helpers = root.grain_core.json_helpers;
...
test "api server parse json string from request" {
    var server = root.grain_core.api_server.ApiServer.init(8080);
    var req = root.grain_core.api_server.HttpRequest.init();
...
test "api server write json string to response" {
    var server = root.grain_core.api_server.ApiServer.init(8080);
    var res = root.grain_core.api_server.HttpResponse.init();

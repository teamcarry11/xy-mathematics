...
const SystemTrayManager = grain_os.system_tray.SystemTrayManager;
...
test "system tray constants" {
    std.debug.assert(grain_os.system_tray.MAX_TRAY_ICONS == 32);
    std.debug.assert(grain_os.system_tray.MAX_TOOLTIP_LEN == 128);
    std.debug.assert(grain_os.system_tray.MAX_ICON_PATH_LEN == 256);
}

...
const ProcessManager = grain_core.process_manager.ProcessManager;
const ProcessState = grain_core.process_manager.ProcessState;
const ProcessPriority = grain_core.process_manager.ProcessPriority;
...
test "process manager constants" {
    std.debug.assert(grain_core.process_manager.MAX_PROCESSES == 256);
    std.debug.assert(grain_core.process_manager.MAX_PROCESS_NAME_LEN == 128);
    std.debug.assert(grain_core.process_manager.MAX_CMD_LINE_LEN == 512);
}

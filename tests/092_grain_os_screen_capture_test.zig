...
const ScreenCaptureManager = grain_os.screen_capture.ScreenCaptureManager;
const CaptureFormat = grain_os.screen_capture.CaptureFormat;
const CaptureType = grain_os.screen_capture.CaptureType;
...
test "screen capture constants" {
    std.debug.assert(grain_os.screen_capture.MAX_CAPTURES == 16);
    std.debug.assert(grain_os.screen_capture.MAX_CAPTURE_NAME_LEN == 128);
}

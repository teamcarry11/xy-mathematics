...
const AudioManager = grain_os.audio_manager.AudioManager;
const AudioDeviceType = grain_os.audio_manager.AudioDeviceType;
...
test "audio manager constants" {
    std.debug.assert(grain_os.audio_manager.MAX_AUDIO_DEVICES == 16);
    std.debug.assert(grain_os.audio_manager.MAX_DEVICE_NAME_LEN == 128);
}
